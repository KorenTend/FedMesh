...
class MultiSizeMeshDataset(Dataset):
    def __init__(self, base_dir="/home/wqq/Gmesh-CNN/Project/GMesh-CNN/cnn_input_data/", group_indices=None, feature_indices=None):
        ...
        for idx in group_indices:
            ...
            if batch_file.exists():
                print(f"   加载组 {idx}: {group_dir.name}")
                ...
                print(f"⚠️  组 {idx} 的 batch_data.pt 不存在，跳过")
...
def prepare_datasets(num_clients, feature_indices, client_group_map):
    print("\n🌐 正在按10个数据源组独立划分 训练集(80%)/验证集(10%)/测试集(10%)...")
    client_train_datasets = {i: [] for i in range(num_clients)}
    server_val_datasets, server_test_datasets = [], []

    for group_id in range(1, 11):
        group_dataset = MultiSizeMeshDataset(base_dir="/home/wqq/Gmesh-CNN/Project/GMesh-CNN/cnn_input_data/", group_indices=[group_id], feature_indices=feature_indices)
